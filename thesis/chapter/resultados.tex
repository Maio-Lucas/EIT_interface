...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/BP_Interface_Frame43.png}
...
No método Back-Projection, mostrado nas Figuras~\ref{fig:bp_frame43} e \ref{fig:bp_frame44}, a região associada à haste pode ser identificada na porção superior-central do domínio, sobretudo pela presença de uma área azul mais intensa. Ainda assim, os contornos aparecem difusos e pouco definidos, o que dificulta a delimitação espacial mais precisa da anomalia. Também se observam regiões avermelhadas na metade inferior da imagem sem correspondência física evidente com o arranjo experimental, caracterizando artefatos do processo de reconstrução. Entre os \textit{frame}s 43 e 44, a posição geral da região dominante se mantém, mas a imagem permanece espalhada e com baixo contraste local.

No método Jacobiano, apresentado nas Figuras~\ref{fig:jac_frame43} e \ref{fig:jac_frame44}, a posição da haste também é reconstruída na região superior-central do domínio, em concordância com o BP, mas com maior definição espacial e contraste mais localizado. No \textit{frame} 43, observa-se um padrão bipolar de alta intensidade, com alternância marcada entre regiões azuis e vermelhas. No \textit{frame} 44, a reconstrução assume uma forma visualmente mais limpa, com predomínio de uma região azul mais bem definida na área associada à anomalia. A comparação entre os dois \textit{frame}s mostra que o método foi capaz de preservar a localização da haste, mas também exibiu maior sensibilidade às variações entre \textit{frames} consecutivos, resultando em mudanças mais bruscas no padrão visual da reconstrução.

O método GREIT, ilustrado nas Figuras~\ref{fig:greit_frame43} e \ref{fig:greit_frame44}, produziu imagens visualmente mais suaves e estáveis entre \textit{frame}s consecutivos. A região de menor condutividade relativa permanece concentrada na parte superior-central do domínio, em posição compatível com a observada nos demais métodos, o que reforça a consistência qualitativa da localização da haste ao longo da reconstrução. Em contrapartida, o contraste é mais baixo e a transição espacial entre regiões é mais gradual, o que reduz a nitidez dos contornos. Também se destaca a presença de uma borda retangular visível na área reconstruída, decorrente da saída do método em grade regular de pixels. Em conjunto, os três algoritmos localizaram a anomalia na mesma região do domínio, diferindo principalmente na definição dos contornos, no contraste e na estabilidade visual entre \textit{frame}s consecutivos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/BP_Interface_Frame43.png}
    \caption{Reconstrução obtida com o método Back-Projection (BP) no \textit{frame} 43. Parâmetros: \(h_0 = 0{,}10\), \(\alpha = 0{,}40\), colormap \texttt{RdBu\_r}.}
    \label{fig:bp_frame43}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/BP_Interface_Frame44.png}
    \caption{Reconstrução obtida com o método Back-Projection (BP) no \textit{frame} 44. Parâmetros: \(h_0 = 0{,}10\), \(\alpha = 0{,}40\), colormap \texttt{RdBu\_r}.}
    \label{fig:bp_frame44}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/JAC_Interface_Frame43.png}
    \caption{Reconstrução obtida com o método Jacobiano (JAC) no \textit{frame} 43. Parâmetros: \(h_0 = 0{,}10\), regularização \(\lambda = 0{,}0100\), método \texttt{kotre}, colormap \texttt{RdBu\_r}.}
    \label{fig:jac_frame43}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/JAC_Interface_Frame44.png}
    \caption{Reconstrução obtida com o método Jacobiano (JAC) no \textit{frame} 44. Parâmetros: \(h_0 = 0{,}10\), regularização \(\lambda = 0{,}0100\), método \texttt{kotre}, colormap \texttt{RdBu\_r}.}
    \label{fig:jac_frame44}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/GREIT_Interface_Frame43.png}
    \caption{Reconstrução obtida com o método GREIT no \textit{frame} 43. Parâmetros: \(h_0 = 0{,}10\), resolução da grade \(= 128\), colormap \texttt{RdBu\_r}.}
    \label{fig:greit_frame43}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{pictures/GREIT_Interface_Frame44.png}
    \caption{Reconstrução obtida com o método GREIT no \textit{frame} 44. Parâmetros: \(h_0 = 0{,}10\), resolução da grade \(= 128\), colormap \texttt{RdBu\_r}.}
